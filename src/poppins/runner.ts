 ...
 */
const DEFAULT_FINAL_HANDLER = async () => {
 ...
 * ```
 */
export class Runner<MiddlewareFn> {
    /**
    ...
    /**
     * Invoke one or another
     */
    #invoker: (self: Runner<MiddlewareFn>) => void | Promise<void>;

    constructor(middleware: MiddlewareFn[]) {
        this.#middleware = middleware;
        this.#invoker = this.#invoke;
    }
     ...
     */
    #invokeWithErrorManagement(
        self: Runner<MiddlewareFn>
    ): Promise<void> | void {
        const middleware = self.#middleware[self.#currentIndex++];
        debug('running middleware at index', self.#currentIndex);
    ...
    errorHandler(errorHandler: ErrorHandler): this {
        ...
        this.#invoker = this.#invokeWithErrorManagement;
        return this;
    }
    ...
    async run(cb: Executor<MiddlewareFn>): Promise<void> {
        this.#executor = cb;
        debug(
            'starting middleware chain with %d middleware',
            this.#middleware.length
        );

        await this.#invoker(this);
    }
}
